...
public class TinyDB {
    ...
    public void putListMeasurements(String key, ArrayList<Measurement> measurementsList){
        checkForNullKey(key);
        Gson gson = new Gson();
        ArrayList<String> objStrings = new ArrayList<String>();
        for(Measurement measurement: measurementsList){
            objStrings.add(gson.toJson(measurement));
        }
        putListString(key, objStrings);
    }

    //No need Class<?> mClass parameter. Because we know it is Player!
    public ArrayList<Measurement> getListMeasurements(String key){
        Gson gson = new Gson();
        ...
        for(String jObjString : objStrings){
            Measurement value  = gson.fromJson(jObjString,  Measurement.class);
